...
import { CartResponse, PlanName } from "@/types/types";
...
export default function Component() {
  ...
  const handleDeleteAddress = (id: number) => {
    ...
    if (selectedAddress === id) {
      setSelectedAddress(
        addresses.length > 1 ? addresses.find((a) => a.id !== id)?.id || 0 : 0
      );
    }
  };
                  ...
                  {addresses.map((address) => (
                    <div key={address.id} className="relative">
                      <Label
                        ...
                        className="flex items-center space-x-3 border p-4 rounded-md cursor-pointer hover:bg-muted"
                      >
                        <RadioGroupItem
